...
namespace ServiceClaim.Models
{
    public class Classifier : DbModel
    {
        ...
        public static void SaveFromExcel(byte[] data, string creatorSid)
        {
            MemoryStream ms = new MemoryStream(data);
            var wb = new XLWorkbook(ms);

            Classifier.SaveFromExcel(wb, creatorSid);
        ...
        public static void SaveFromExcel(XLWorkbook wb, string creatorSid)
        {
            ...
            foreach (var row in ws.Rows())
            {
                ...
                var cat = new ClassifierCaterory() { Name = catName, Number = catNumber, Complexity = catComplexity };
                cat.Save(creatorSid);
                ...
                while (!String.IsNullOrEmpty(ws.Row(headerRow).Cell(c).Value.ToString()))
                {
                    ...
                    var clItem = new ClassifierItem() { IdCategory = idCategory, IdWorkType = idWorkType };
