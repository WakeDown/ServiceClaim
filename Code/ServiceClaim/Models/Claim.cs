...
namespace ServiceClaim.Models
{
    public class Claim:DbModel
    {
        ...
        //для формы новой записи
        public string ctrFind { get; set; }
        ...
        /// <returns></returns>
        public static async Task<int> SaveFromServicePlan4ZipClaim(AdUser user, int idServiceCame)
        {
            ...
            claim.IdWorkType = WorkType.GetWorkTypeForZipClaim().Id;
            ...
            int id = claim.Save(user.Sid, firstState);
            await claim.Go(user);
            return id;
        }
        /// <summary>
        ...
        /// <returns></returns>
        public int Save(string creatorSid, ClaimState firstState = null)
        {
            bool isNew = Id == 0;
            ...
            if (isNew)
            {
                ...
                SaveStateStep(st.Id, creatorSid, Descr);
            }
        ...
        public void SaveStateStep(int stateId, string creatorSid, string descr = null, bool saveStateInfo = true, int? idZipClaim = null, bool saveClaimCurrentState = true)
        {
            ...
            c2Cs.Save(creatorSid, saveClaimCurrentState);
        }
        ...
        private bool UserIsCurrentClaimRole(ServiceRole role, string creatorSid, bool roleOrSpecialist = false)
        {
            if (String.IsNullOrEmpty(creatorSid)) return false;
            bool result = false;
            ...
            {
                return (roleOrSpecialist && cl.SpecialistSid == creatorSid) || cl.CurTechSid == creatorSid;
            }
            else if (role == ServiceRole.CurAdmin)
            {
                return (roleOrSpecialist && cl.SpecialistSid == creatorSid) || cl.CurAdminSid == creatorSid;
            }
            else if (role == ServiceRole.CurManager)
            {
                return (roleOrSpecialist && cl.SpecialistSid == creatorSid) || cl.CurManagerSid == creatorSid;
            }
            else if (role == ServiceRole.CurEngeneer)
            {
                return (roleOrSpecialist && cl.SpecialistSid == creatorSid) || cl.CurEngeneerSid == creatorSid;
            }
            else if (role == ServiceRole.CurEngeneer)
            {
                return (roleOrSpecialist && cl.SpecialistSid == creatorSid) || cl.SpecialistSid == creatorSid;
            }
            else if (role == ServiceRole.CurSpecialist)
            {
                return cl.SpecialistSid == creatorSid;
            }
        ...
        public async Task Go(AdUser user, bool confirm = true)
        {
            ...
            else if (currState.SysName.ToUpper().Equals("TECHWORK"))
            {
                ...
                if (confirm)
                {
                    ...
                    if (ServiceSheet4Save.ProcessEnabled.HasValue && ServiceSheet4Save.ProcessEnabled.Value && ServiceSheet4Save.DeviceEnabled.HasValue && ServiceSheet4Save.DeviceEnabled.Value)
                    {
                        ...
                        SaveStateStep(nextState.Id, user.Sid);
                        //saveStateInfo = false;
                        ...
                        if (GetOrderedZipItemList(ServiceSheet4Save.Id).Any(x => !x.Installed))
                        {
                            nextState = new ClaimState("SERVADMSETWAIT");
                        }
                    }
                    else if (((!ServiceSheet4Save.ProcessEnabled.HasValue || !ServiceSheet4Save.ProcessEnabled.Value) || (!ServiceSheet4Save.DeviceEnabled.HasValue || !ServiceSheet4Save.DeviceEnabled.Value)) &&
            ...
            else if (currState.SysName.ToUpper().Equals("SRVADMWORK"))
            {
                ...
                int serviceIssueId = ServiceIssue4Save.Save(user.Sid);
            ...
            else if (currState.SysName.ToUpper().Equals("SRVENGWORK"))
            {
                ...
                bool hasNotInstalled = GetOrderedZipItemList(ServiceSheet4Save.Id).Any(x => !x.Installed);
            ...
            else if (currState.SysName.ToUpper().Equals("CONTRACTSET"))
            {
                ...
                if (lastServiceSheet.ProcessEnabled.HasValue && lastServiceSheet.ProcessEnabled.Value && lastServiceSheet.DeviceEnabled.HasValue && lastServiceSheet.DeviceEnabled.Value)
                {
                    nextState = new ClaimState("DONE");
                }
            ...
            else if (currState.SysName.ToUpper().Equals("ZIPISSUE"))
            {
                ...
                goNext = true;
                saveClaim = true;
                var notInstalledList = GetOrderedZipItemList(lastServiceSheet.Id).Where(x => !x.Installed);
                ...
                if (lastServiceSheet.ZipClaim.HasValue && lastServiceSheet.ZipClaim.Value)
                {
                    ...
                    {
                        ServiceSheetZipItem.SaveOrderedZipItemsCopyFromIssued(lastServiceSheet.Id, user.Sid);
            ...
            if (sendNote)
            {
                ...
                await SendNote(noteSubject, noteText, null, noteTo);
        ...
        public static async Task ServiceSheetIsPayWraped(AdUser user, int serviceSheetId)
        {
            ...
            if (ss.IsPayed.HasValue)
            {
                ...
                await cl.SendNote(subject, text, serviceSheetId, ServiceRole.ServiceSheetEngeneer);
        ...
        public async Task SendNote(string subject, string text, int? serviceSheetId = null, params ServiceRole[] to)
        {
           ...
           await SendMailTo(text, subject, serviceSheetId, to);
        }
        ...
        public async Task SendMailTo(string message, string subject, int? serviceSheetId = null, params ServiceRole[] mailTo)
        {
            var cl = new Claim(Id, loadObject: false);
            ...
            foreach (ServiceRole mt in mailTo)
            {
                ...
                if (mt == ServiceRole.CurAdmin)
                {
                    ...
                    string email = EmployeeSm.GetEmailBySid(sid);
                ...
                else if (mt == ServiceRole.AllTech)
                {
                    ...
                    {
                        string email = EmployeeSm.GetEmailBySid(item.Key);
                        if (!String.IsNullOrEmpty(email))
                        {
                            recipients.Add(new MailAddress(email));
                        }
        ...
        public IEnumerable<ServiceSheetZipItem> GetOrderedZipItemList(int? serviceSheetId = null)
        {
            SqlParameter pClaimId = new SqlParameter() { ParameterName = "claim_id", SqlValue = Id, SqlDbType = SqlDbType.Int };
        ...
        public SelectList GetCurrentClaimSpecialistSelectionList()
        {
            ...
            //string jsonString = GetJson(uri);
            //var model = JsonConvert.DeserializeObject<IEnumerable<KeyValuePair<string, string>>>(jsonString);
            var model = GetSpecialistList(Id);
        ...
        {
            return Claim.GetClaimServiceSheetList(Id, payedWrap);
